...
PUBLIC_GET_ROUTES = {
    ...
    "/api/provenance",
    "/api/methodology",
    # The triage routes exist in both modes and are admitted only for the pinned
    # approved release, so they belong on the allowlist and are covered by their
    # own boundary test.
    "/api/triage/period",
    "/api/triage/evidence",
    "/api/triage/queue",
...
def test_public_contracts_are_typed_bounded_and_synthetic() -> None:
    ...
    methodology = client.get("/api/methodology").json()
    assert "approved six-case replay only" in methodology["research_boundary"]
    assert "larger triage artifact stay local" in methodology["research_boundary"]
    assert "elliptic" not in methodology
    assert "Elliptic" not in methodology["research_boundary"]
