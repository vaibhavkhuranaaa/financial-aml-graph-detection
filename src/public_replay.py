...
def approved_public_artifact(
    ...
    *,
    ...
    if expected_sha256 and artifact.get("artifact_sha256") != expected_sha256:
        raise ValueError("Public fixture does not match the approved release digest.")
    if (
        expected_pipeline_run_id
        and provenance.get("pipeline_run_id") != expected_pipeline_run_id
    ):
        raise ValueError("Public fixture does not match the approved pipeline run.")
    return artifact
