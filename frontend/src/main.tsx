...
function Masthead({ triage = false }: { triage?: boolean }) {
  return (
    <header className="masthead">
      <div>
        <p className="stamp">
          {triage
            ? "Approved triage period · realistic synthetic banking data"
            : "Approved public replay · realistic synthetic banking data"}
        </p>
        <h1>Signal Ledger</h1>
      </div>
      <p className="mast-meta">
        <i />{" "}
        {triage
          ? "One approved review period · no live feed · no visitor inference"
          : "Deterministic replay · no live feed · no visitor inference"}
      </p>
    </header>
  );
}
  ...
  return (
    <main>
      ...
      <section className="opening" aria-labelledby="workbench-title">
        <div>
          <p className="stamp">Six-case evidence replay</p>
          <h2 id="workbench-title">Trace the evidence. Keep the decision human.</h2>
          <p>Review 55 transactions across six approved synthetic cases. Time, counterparties, evidence, and limits stay visible before any simulated record. Nothing here identifies a person, produces a finding, or recommends a compliance action.</p>
        </div>
      ...
      <section id="replay" className="workbench" aria-label="Synthetic case investigation">
        ...
        <section className="replay-panel" aria-labelledby="replay-title">
          <div className="panel-heading">
            ...
            <span>{timeline.length ? `${cursor + 1} / ${timeline.length}` : "n/a"}</span>
          </div>
      ...
      <section className="audit-section" aria-labelledby="audit-title">
        ...
        <div className="audit-workspace">
          <form className="audit-form" onSubmit={(event) => event.preventDefault()}>
            <div className="panel-heading"><div><p className="stamp">Local-only entry</p><h2>{detail?.outcome ?? "Select a case"}</h2></div><span>{provenance ? `v${provenance.version}` : "n/a"}</span></div>
      ...
      <section className="method" aria-labelledby="provenance-title">
        ...
        <dl><div><dt>Provider</dt><dd>{provenance?.provider}</dd></div><div><dt>Source checksum</dt><dd>{provenance?.source_sha256}</dd></div><div><dt>Replay checksum</dt><dd>{provenance?.slice_sha256}</dd></div><div><dt>Public product boundary</dt><dd>This API serves the approved six-case replay and one approved review period of the triage queue. Raw source data, the alert store, the feature table, tuned rule parameters, model execution, the LI-Small variant, and every other review period remain local.</dd></div></dl>
      </section>
    </main>
  );
}
...
function TriageApp() {
  return (
    <main>
      <Masthead triage />
      <section className="local-triage-intro" aria-label="Triage scope boundary">
        <p className="stamp">One approved review period</p>
        <p>The owner recorded a distribution decision for this review period of the HI-Small variant, and for nothing else. Every other period, the alert store, the feature table, the LI-Small variant and the tuned rule parameter values stay local. The service admits only the pinned release of this artifact and refuses any other build.</p>
      </section>
      <TriageDesk />
    </main>
  );
...
createRoot(document.getElementById("root")!).render(surface === "triage" ? <TriageApp /> : <App />);
